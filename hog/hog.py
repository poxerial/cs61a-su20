...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    def one_turn(strategy, score, opponent_score):
        num_rolls = strategy(score, opponent_score)
        outcome = take_turn(num_rolls, score, opponent_score, dice)
        return hog_pile(score + outcome, opponent_score) + outcome
    while max(score0, score1) < goal:
        if who == 0:
            score0 += one_turn(strategy0, score0, score1)
        else:
            score1 += one_turn(strategy1, score1, score0)
        who = next_player(who)
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if current_leader != None:
        if last_leader != current_leader:
            message = f'Player {current_leader} takes the lead by {abs(score0 - score1)}'
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_roll_zero = score + take_turn(0, score, opponent_score)
    if hog_pile(score_roll_zero, opponent_score) != 0:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return hog_pile_strategy(score, opponent_score, 9, 4)
    # END PROBLEM 12
##########################
